      ...
      TxtDescription.Text = string.IsNullOrWhiteSpace(metadata.Description) ? "No description provided." : metadata.Description;
          TxtCreatedDate.Text = _shareableProject.CreatedDate.ToLocalTime().ToString("MMMM d, yyyy");
           TxtModrixVersion.Text = _shareableProject.ModrixVersion;
            ...
            // Content counts - adjust based on project type
        if (_shareableProject.IsResourcePack)
                {
     // Resource pack content
     var rpData = _shareableProject.ResourcePackData;
             int textureCount = rpData?.TextureOverrides?.Count ?? 0;
     int modelCount = rpData?.ModelOverrides?.Count ?? 0;
              int soundCount = rpData?.SoundOverrides?.Count ?? 0;
                  int translationCount = rpData?.TranslationOverrides?.Count ?? 0;

      TxtModElementsCount.Text = $"{textureCount} texture overrides";
     TxtResourcesCount.Text = $"{modelCount} model overrides";
           TxtCodeFilesCount.Text = $"{soundCount} sound overrides";
      TxtLanguagesCount.Text = $"{_shareableProject.Languages.Count + translationCount} languages/translations";
     }
         else
 {
      // Mod project content
...
Title = "Invalid File",
  ...
  if (_shareableProject.IsResourcePack)
 {
        // Import as resource pack - let ShareableProjectService handle it
         UpdateProgress(progress, "Importing resource pack...", 20);
        project = await _shareService.ImportProjectAsync(_modrixFilePath, projectsBasePath, progress);
}
                else
   {
       // Import as mod project - need to create template first
                    UpdateProgress(progress, "Creating project structure...", 20);

       project = new ModProjectData
       {
       Name = metadata.Name,
 ...
 IncludeReadme = metadata.IncludeReadme
  ...
  Content = $"{(_shareableProject.IsResourcePack ? "Resource pack" : "Project")} '{metadata.Name}' has been imported successfully!\n\n" +
